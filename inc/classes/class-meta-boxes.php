...
class Meta_Boxes {
	...
	public function add_custom_meta_box() {
		...
		foreach ( $screens as $screen ) {
			add_meta_box(
				...
				__( 'Hide page title', 'duam' ),  // Box title
	...
	public function custom_meta_box_html( $post ) {
		...
		?>
		<label for="duam-field"><?php esc_html_e( 'Esconder el título de la página', 'duam' ); ?></label>
		<select name="duam_hide_title_field" id="duam-field" class="postbox">
			<option value=""><?php esc_html_e( 'Selección', 'duam' ); ?></option>
			<option value="yes" <?php selected( $value, 'yes' ); ?>>
				<?php esc_html_e( 'Si', 'duam' ); ?>
			</option>
			<option value="no" <?php selected( $value, 'no' ); ?>>
				<?php esc_html_e( 'No', 'duam' ); ?>
			</option>
		</select>
		<?php
	}
	...
	public function save_post_meta_data( $post_id ) {
		...
		if ( array_key_exists( 'duam_hide_title_field', $_POST ) ) {
			update_post_meta(
				$post_id,
				'_hide_page_title',
				$_POST['duam_hide_title_field']
			);
		}
	}
